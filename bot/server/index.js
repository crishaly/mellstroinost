...
function applyTimeDecay(pet) {
  ...
  const penalty = (hunger < 30 ? 8 : 0) + (cleanliness < 30 ? 6 : 0) + (energy < 30 ? 8 : 0);
...
// 1) Авторизация: фронт присылает initData -> сервер проверяет подпись -> создаёт user+pet
...
// 2) Получить состояние (сервер пересчитает по времени)
